...
import '/models/teacher_models/solution_model.dart';
import '../../db_helper/secure_helper/secure_helper_login.dart';

class SolutionController with ChangeNotifier {
  final SecureHelperLogin _secureHelperLogin = SecureHelperLogin();

  SolutionModel _solutionModel = SolutionModel();
  SolutionModel get solutionModel => _solutionModel;
  ...
  void readSolution() async {
    try {
      ...
      if (cache == _cacheSolution.noSolution) {
        ...
        _solutionModel = await compute(pareJsonSolution, cache);
  ...
  Future<bool> createSolution(String text, String file) async {
    try {
      var token = await _secureHelperLogin.readUserLogin();
...
SolutionModel pareJsonSolution(String jsonString) =>
    SolutionModel.fromJson(json.decode(jsonString));
